...
Step 1: Password + honeypot bot trap
Step 2: Math CAPTCHA
...
def require_auth():
    ...
    st.markdown(
        '<div class="sep-footer">✦ Authorised access only &nbsp;·&nbsp; Protected by custom CAPTCHA</div>',
        unsafe_allow_html=True
    )
    st.stop()
...
def _step1():
    ...
    st.markdown(
        '<div class="sep-step-label">Step 1 of 2 &nbsp;·&nbsp; Access Key</div>',
        unsafe_allow_html=True
    )
    ...
    if btn:
        if hp:  # bot filled the honeypot
            st.markdown(
                '<div class="sep-err">🤖 Automated access detected.</div>',
                unsafe_allow_html=True
            )
        elif not pwd:
            st.markdown(
                '<div class="sep-err">Please enter your access key.</div>',
                unsafe_allow_html=True
            )
...
def _step2():
    ...
    st.markdown('<div class="sep-card">', unsafe_allow_html=True)
    st.markdown(
        "<div class=\"sep-step-label\">Step 2 of 2 &nbsp;·&nbsp; Verify You're Human</div>",
        unsafe_allow_html=True
    )
    st.markdown(f"""
    ...
    user_ans = st.text_input(
        ...
        label_visibility="collapsed", key="_cap_input"
    )
    ...
    with c2:
        refresh_btn = st.button("New ↺", use_container_width=True, key="_cap_refresh")
    ...
    if verify_btn:
        try:
            ...
            st.markdown(
                '<div class="sep-err">Please enter a number.</div>',
                unsafe_allow_html=True
            )
        else:
            if given == ans:
...
def _css() -> str:
    ...
    /* Password input — normal, fully visible */
    .stTextInput > div > div > input {
        background:rgba(255,255,255,.04)!important;
        border:1px solid rgba(201,168,76,.22)!important;
        border-radius:10px!important;color:#fff!important;
        font-family:'Space Grotesk',sans-serif!important;
        font-size:1rem!important;letter-spacing:4px!important;
