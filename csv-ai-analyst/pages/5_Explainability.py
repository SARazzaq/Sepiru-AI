...
Task: {results['task']}
Performance: {perf}
Top features by importance: {feat_imp or 'not available'}
Dataset columns: {list(df.columns)}
Dataset shape: {df.shape}

Question: {q}

Answer in {lang}. Be specific, use the actual feature names and numbers. 
Make it understandable for the stated audience."""

            answer = ai.generate(prompt, system="You are an expert ML explainability specialist.")
...
st.info("Fill in values below to get a prediction with explanation.")
...
if model:
    ...
    if num_cols:
        ...
        for i, col in enumerate(num_cols[:8]):
            with cols[i % 4]:
                vals[col] = st.number_input(col, value=float(df[col].median()), key=f"exp_{col}")
        ...
        if st.button("Predict this row", use_container_width=True):
            try:
                ...
                if can_proceed():
                    ai = AIClient()
                    exp_prompt = f"""Model predicted: {pred}
Input values: {vals}
Model: {results['model_name']}
Explain in 2-3 sentences why this prediction makes sense given the input values. Use {lang}."""
